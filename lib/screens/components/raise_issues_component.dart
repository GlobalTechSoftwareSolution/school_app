...
class RaiseIssuesComponent extends StatefulWidget {
  const RaiseIssuesComponent({super.key});

  @override
  State<RaiseIssuesComponent> createState() => _RaiseIssuesComponentState();
}

class _RaiseIssuesComponentState extends State<RaiseIssuesComponent> {
  List<Issue> issues = [];
  bool loading = false;
  bool showForm = false;
  bool showView = false;
  ...
  Future<void> fetchIssues() async {
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      final studentEmail = prefs.getString('user_email');

      if (studentEmail == null) {
        throw Exception("Please log in to access your issues");
      }

      final res = await http.get(
        Uri.parse(
          'https://school.globaltechsoftwaresolutions.cloud/api/issues/',
        ),
      );

      final data = json.decode(res.body) as List;
      final allIssues = data.map((i) => Issue.fromJson(i)).toList();

      // Filter issues for this student
      final studentIssues = allIssues
          .where(
            (issue) =>
                issue.raisedBy?.toLowerCase() == studentEmail.toLowerCase(),
          )
          .toList();

      setState(() => issues = studentIssues);
    } catch (err) {
  ...
  Future<void> handleStatusChange(int id, String newStatus) async {
    try {
      await http.patch(
        Uri.parse(
          'https://school.globaltechsoftwaresolutions.cloud/api/issues/$id/',
        ),
        headers: {'Content-Type': 'application/json'},
  ...
  Future<void> handleAddComment() async {
    ...
    try {
      ...
      await http.patch(
        Uri.parse(
          'https://school.globaltechsoftwaresolutions.cloud/api/issues/${selectedIssue!.id}/',
        ),
        headers: {'Content-Type': 'application/json'},
